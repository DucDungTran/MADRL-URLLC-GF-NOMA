...
class DQN():
    def __init__(self, n_agents, state_size, n_actions, tau, alpha, gamma, epsilon, epsilon_min,
                 epsilon_decay, target_update_freq, memory_size, batch_size, mode_update):
    ...
    def learn(self):
        for agent_idx in range(self.n_agents):
            ...
            q_model_st[range(self.batch_size), action_batch] = reward_batch + self.gamma*np.max(q_target_nst, axis=1)
